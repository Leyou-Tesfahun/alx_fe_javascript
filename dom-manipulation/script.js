...
async function fetchQuotesFromServer() {
  try {
    ...
    // Simulate server quotes structure, take first 5 posts as quotes
    const serverQuotes = data.slice(0, 5).map(post => ({
      text: post.title,
      category: 'Server'
    }));

    // Conflict resolution: server data overwrites local quotes with same text
    let updated = false;
...
// Sync quotes: post local quotes to server (simulated)
...
function init() {
  ...
  // Periodic syncing every 1 minute
